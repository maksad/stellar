...
class Snapshot(Base):
    ...
    __table_args__ = {"schema": STELLAR_SCHEMA}
    id = sa.Column(
        sa.Integer,
        sa.Sequence("snapshot_id_seq", schema=STELLAR_SCHEMA),
        primary_key=True,
    )
...
class Table(Base):
    ...
    __table_args__ = {"schema": STELLAR_SCHEMA}
    id = sa.Column(
        sa.Integer, sa.Sequence("table_id_seq", schema=STELLAR_SCHEMA), primary_key=True
    )
