...
def list():
    ...
    click.echo(
        "\n".join(
            "%s: %s" % (s.snapshot_name, humanize.naturaltime(utc_now() - s.created_at))
            for s in snapshots
        )
    )
...
def _select_postgres_schema(conn):
    schemas = [s for s in list_of_databases(conn) if not s.startswith("stellar_")]
    if database_exists(conn, "public") and "public" not in schemas:
        schemas = ["public"] + schemas
    if not schemas:
        click.echo("Could not find any schemas in this database.")
        sys.exit(1)
    if len(schemas) == 1:
        click.echo("Using schema: %s" % schemas[0])
        return schemas[0]
    while True:
        click.echo("You have the following schemas: %s" % ", ".join(schemas))
        schema_name = click.prompt(
            "Please enter the schema to snapshot (eg. public)",
            default="public" if "public" in schemas else schemas[0],
        )
        if database_exists(conn, schema_name):
            return schema_name
        click.echo("Could not find schema %s" % schema_name)
        click.echo("")
...
def init():
    ...
    while True:
        ...
        try:
            engine = create_engine(
                connection_url, echo=False, isolation_level="AUTOCOMMIT"
            )
            conn = engine.connect()
        except OperationalError as err:
            click.echo("Could not connect to database: %s" % url)
            click.echo("Error message: %s" % str(err))
            click.echo("")
            continue
        except ArgumentError as err:
            click.echo("Could not parse database url: %s" % url)
            click.echo("Error message: %s" % str(err))
            click.echo("")
            continue
        else:
    ...
    if url.count("/") == 3 and url.endswith("/"):
        while True:
            if engine.dialect.name == "postgresql":
                listed = list_of_cluster_databases(conn)
            else:
                listed = list_of_databases(conn)
            click.echo(
                "You have the following databases: %s"
                % ", ".join([db for db in listed if not db.startswith("stellar_")])
            )
            ...
            if engine.dialect.name == "postgresql":
                exists = cluster_database_exists(conn, db_name)
            else:
    ...
    with open("stellar.yaml", "w") as project_file:
        project_file.write(
            """
project_name: '%(name)s'
tracked_databases: ['%(tracked_name)s']
url: '%(raw_url)s'
stellar_url: '%(stellar_url)s'
            """.strip()
            % {
                "name": name,
                "raw_url": raw_url,
...
def main():
    try:
        ...
        click.echo("Your stellar.yaml configuration is wrong: %s" % str(e))
        sys.exit(1)
